...
public class CarController : MonoBehaviour 
{
    ...
    void FixedUpdate()
    {
        if (drive)
        {
            right = Mathf.Clamp(Input.GetAxis(ControllerInputs.righttrigger), 0, 1);
            motor = maxTorque * right;
            left = Mathf.Clamp(Input.GetAxis(ControllerInputs.lefttrigger), 0, 1);
            ...
            steering = maxSteeringAngle * Input.GetAxis(ControllerInputs.leftjoystickhorizontal);
